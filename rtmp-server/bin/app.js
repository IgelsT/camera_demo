#!/usr/bin/env node
const AuthFromAPI = require("./auth.js");
const authFromAPI = new AuthFromAPI();
...
const config = {
    ...
    http: {
        ...
        mediaroot: "/var/www-data/camera.imile.ru/public_html/hls",
...
nms.on("prePublish", (id, StreamPath, args) => {
    ...
    let session = nms.getSession(id);
    // console.log(session.isLocal);
    authFromAPI.authPublish(args).catch((e) => {
...
nms.on("prePlay", (id, StreamPath, args) => {
    ...
    let session = nms.getSession(id);
    if (session.isLocal == true) return;

    console.log("External");
    authFromAPI.authPlay(args).catch((e) => {
        // console.log(session);
        if (typeof session === "NodeRtmpSession") {
            session.sendStatusMessage(
                id,
                "error",
                "NetConnection.Connect.Rejected",
