...
class ERROR_CODES
{
    ...
    static array $PARAM_REQUIRED = ['PARAM_REQUIRED', 'param required', 422];
    ...
    static array $USER_NOT_CONFIRM = ['NOT_CONFIRMED', 'Аккаунт не подтвержден!', 401];
...
class ApiException extends Exception
{
    ...
    public function render(Request $request): JsonResponse
    {
        ...
        return response()->json(["isError" => true, "error" => $error], $this->httpCode);
    }
    ...
    public function toArray(): array
    {
        $str = "{$this->message} in {$this->file}:{$this->line}";
        $trace = $this->getTrace();
        array_unshift($trace, $str);
        return $trace;
    }

    public static function fromCODE(array $params, string $reason = ''): ApiException
    {
        return new self($params, $reason);
    }

    public static function fromException(\Throwable $exception): ApiException
    {
